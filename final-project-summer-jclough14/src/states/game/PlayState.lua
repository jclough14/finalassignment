...
function PlayState:enter(params)
    ...
    self.texture = 'green-alien'

    self.player = Player({
        x = 0, y = 0,
        width = 16, height = 20,
        texture = self.texture,
        powerupMachine = StateMachine {
            ['normal'] = function() return PlayerNormalState(self.player) end,
            ['fire'] = function() return PlayerFireState(self.player, self.gravityAmount) end,
        },
    ...
    self.player.plevel = params.plevel or 1

    self:spawnEnemies()

    self.player:changeState('falling')
    local duration = {fireDuration = 100}
    --self.player:changepowerupState('fire', duration) DEBUG
...
function PlayState:render()
    ...
    -- render level
    love.graphics.setColor(0, 0, 0, 255)
    love.graphics.print('Level: ' .. '' .. tostring(self.player.plevel), 190, 5)
    love.graphics.setColor(255, 255, 255, 255)
    love.graphics.print('Level: ' .. '' .. tostring(self.player.plevel), 189, 4)
end
